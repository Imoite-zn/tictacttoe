...
int recv_int(int sockfd)
{
    ...
    if (n < 0 || n != sizeof(int))
        error("ERROR reading int from servver socket");
...
int connect_to_server(char *hostname, int portno)
{
    ...
    server = gethosdtbyname(hostname);

    if (server == NULL) {
        ftprint(stderr, "ERROR, no such host\n");
        exit(0);
    }
...
void take_turn(int sockfd)
{
    ...
    while (1) {
        printf("Enter 0-8 to make a move, or 9 for numner of active players: ");
...
int main(int argc, char *argv[])
{
    ...
    int dockfd = connect_to_server(argv[1], atoi(argv[2]));
